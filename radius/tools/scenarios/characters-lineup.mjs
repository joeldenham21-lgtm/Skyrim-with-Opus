...
const CLASSES = ['recruit', 'regular', 'veteran', 'elite', 'sniper', 'gunner'];
...
export default async function (page, api) {
  ...
  const info = await api.run(`(() => { const r = window.__radius, ctx = r.ctx, p = ctx.player; p.update(0); const f = p.forward;
    ...
    for (let i = 0; i < cls.length; i++) {
      const side = (i - (cls.length - 1) / 2) * 1.35;
      const x = p.position.x + f.x * 7.5 + f.z * side, z = p.position.z + f.z * 7.5 - f.x * side;
  ...
  await api.run(`(() => { const r = window.__radius, ctx = r.ctx, p = ctx.player; ctx.enemies.removeAll(); p.update(0); const f = p.forward;
    const e = r.spawn('mimic', p.position.x + f.x * 3.2 + f.z * 0.3, p.position.z + f.z * 3.2 - f.x * 0.3, { cls: 'veteran', tide: 3, idle: true, yaw: p.yaw + Math.PI + 0.7 });
  ...
  // ---- aiming pose at 6 m
  await api.run(`(() => { const r = window.__radius, ctx = r.ctx, p = ctx.player, m = window.__m; m.position.set(p.position.x + p.forward.x * 6, m.position.y, p.position.z + p.forward.z * 6); m.aware = 1; m.engaged = true; m.setState('engage'); m.lastVisT = ctx.elapsed; m.lastSeenPlayer = p.position.clone();
