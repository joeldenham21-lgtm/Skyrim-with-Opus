// Review pass 3 (after fixes): ribbon arcs, glass shards, sulphur fog; damage and post numbers; night looks.
export default async function (page, api) {
  ...
  const info = async () => api.run(`(() => { const r = window.__radius, s = r.stats(); const u = r.ctx.post.uniforms; return { hp: s.hp, calls: s.calls, err: s.error, distort: +u.uDistort.value.toFixed(3), ab: +u.uAberration.value.toFixed(3), blur: +u.uBlur.value.toFixed(3), pos: s.pos, an: r.ctx.anomalies.list.map((a) => a.type + (a.revealed ? '*' : '')), missing: s.missingSounds }; })()`);
  const X = 30, Z = 130;
  ...
  await api.run(`window.__radius.teleport(${X}, ${Z - 4.5}); window.__radius.setLook(0, 0.1)`);
  await api.frames(3); await api.screenshot('reflector-close');
  ...
  await api.frames(3); await api.screenshot('reflector-night-revealed');
