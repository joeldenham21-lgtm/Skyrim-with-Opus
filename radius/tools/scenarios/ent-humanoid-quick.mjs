// Four shots only: the fast loop for tuning value and palette. See ent-humanoid-lineup.mjs for the full set.
...
export default async function (page, api) {
  ...
  await look(0.0, 2.1, RX + (1 - 2.5) * 1.15, gy + 1.00, RZ);
  await api.screenshot('ssh60-front');
  await look(0.0, 0.85, RX + (1 - 2.5) * 1.15, gy + 1.62, RZ);
  await api.screenshot('ssh60-head');
  await look(0, 40, RX, gy + 1.0, RZ);
  await api.screenshot('row-40m');
}
