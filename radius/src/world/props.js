...
export function createProps(ctx) {
  ...
  const onRoad = (x, z, m = 0.5) => ROADS.some((r) => distToPolyline(x, z, r.pts).d < r.width * 0.5 + m);
  // structures are built first: keep the heavier dressing out of doorways and off anything already solid
  const doors = (ctx.structures && ctx.structures.doors) || [];
  const nearDoor = (x, z, r) => doors.some((d) => Math.hypot(d.x - x, d.z - z) < r || Math.hypot(d.inside.x - x, d.inside.z - z) < r);
  const clear = (x, z, r) => { for (const [ox, oz] of [[0, 0], [-r, -r], [r, -r], [-r, r], [r, r]]) if (world.pointInSolid(x + ox, world.getHeight(x + ox, z + oz) + 0.6, z + oz)) return false; return true; };
  const placeable = (x, z, r) => dry(x, z) && !onRoad(x, z, r) && !nearDoor(x, z, r + 4.5) && clear(x, z, r);

  // ---- barrels: clusters behind buildings and at the works ----
  const barrelClusters = [
    [-118, 88, 4, 'zarya'], [-152, 30, 3, 'zarya'], [-98, 50, 3, 'zarya'], [131, -56, 6, 'object12'], [172, -44, 4, 'object12'], [131, -42, 3, 'object12'],
  ...
  for (const [cx, cz, n, p] of barrelClusters) {
    ...
    for (let i = 0; i < n * 3 && placed < n; i++) {
      ...
      if (!dry(x, z) || onRoad(x, z) || nearDoor(x, z, 2.6)) continue;
  ...
  for (const [cx, cz, n, ry] of slabPiles) {
    if (!placeable(cx, cz, 1.8)) continue;
  ...
  for (const [cx, cz] of [[140, -84], [152, -44], [-68, -140], [-150, 32]]) {
    if (!placeable(cx, cz, 1.4)) continue;
