...
| `state` | core/state | `state.data` = persistent JSON (hp, stamina, money, securityLevel, day, hour, tideLevel, tideDay, flashlight, inventory, storage, missions, stats, flags, settings, seed). `save()/load()/reset()/saveSettings()/hasSave()`. |
...
- `hands.root` is a Group under the camera; `hands.setWeaponMesh(group|null)`, `hands.kick()`, `hands.adsBlend`. Sway/bob/breath live here. Other modules may parent their own held items (the detector, a thrown-probe hand) under `hands.root`; `setWeaponMesh` must only replace the weapon child it manages, never clear the group.
...
- **Canonical sound names** (sfx agent registers ALL of these; everyone else plays them by name):
  ...
  Extra loops: `heartbeat`, `breath` (played by the player controller). Loop `set()` keys: `fragment_chime` set('rate', hz 0.3–8), set('near', 0..1); `wind` set('gust', 0..1); `radius_hum seeker_hum arc_hum gravity_drone gas_hiss artifact_hum drizzle` set('intensity', 0..1); `mimic_static` set('level', 0..1); `heartbeat`/`breath` set('rate', bpm | breaths/s), set('level').
...
- **Enemy fire contract**: entities shoot with `ctx.ballistics.shoot(origin, dir, { source:'enemy', damage, spreadDeg, tracer:true, kind:'bullet' })`. `ctx.ballistics.isStub` is true until the weapons agent replaces the stub (entities may fall back to their own capsule test meanwhile). With `source:'enemy'` ballistics tests the world and the player capsule (feet `player.position`, radius 0.35, height 1.8 or 1.3 crouched), calls `player.damage(damage, { kind:'bullet', source: opts.shooter })` on a hit, plays `bullet_whiz` when the ray passes within 1.5 m of `player.eye` without hitting, spawns impact vfx, and returns the hit list.
