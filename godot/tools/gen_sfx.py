...
contact sheet). Deterministic: the RNG seed is derived from the name and variant index.

  python3 tools/gen_sfx.py                      # everything (4 processes, ~5 min)
...
def render_one(job):
    ...
    sf.write(path, x.astype(np.float32), dsp.SR, format="OGG", subtype="VORBIS", compression_level=QUALITY)
    return {"name": name, "i": i, "file": fname, "seconds": round(len(x) / dsp.SR, 3), "bytes": os.path.getsize(path),
            "peak_db": round(dsp.to_db(np.max(np.abs(x))), 2), "ms": int((time.time() - t0) * 1000)}
...
def main():
    ...
    if a.list:
        ...
        print("%d names, %d files" % (len(names), sum(catalog.REG[n].variants for n in names)))
        return
    os.makedirs(a.out, exist_ok=True)
    jobs = [(n, i, a.out, catalog.REG[n].variants) for n in names for i in range(catalog.REG[n].variants)]
    print("[gen_sfx] %d names, %d files, %d processes -> %s (modules: %s)" % (len(names), len(jobs), a.jobs, a.out, ", ".join(catalog.LOADED)))
    t0 = time.time()
    written = {}
    errors = []
    total_bytes = 0
    ...
    with Pool(a.jobs) as pool:
        for r in pool.imap_unordered(render_one, jobs, chunksize=2):
            ...
            if "error" in r:
                errors.append(r)
                print("[gen_sfx] ERROR %s_%d: %s" % (r["name"], r["i"] + 1, r["error"].splitlines()[0]))
                continue
    ...
    for n in written:
        written[n].sort()
    # alias copies: legacy names the game may ask for that resolve to another set
    if not a.no_alias_copies:
        ...
        for al, spec in catalog.ALIASES.items():
            ...
            if tgt not in written or al in catalog.REG:
                continue
            if a.only and not any(al.startswith(p) for p in a.only.split(",")):
                continue
            files = []
    ...
    if not a.only:
        ...
        print("[gen_sfx] manifest: %d names, %d files, %.1f MB" % (man["count"], sum(len(f) for f in written.values()), total_bytes / 1e6))
    if errors:
    ...
    if not a.no_report:
        ...
        s = report.run(a.out, None if not a.only else None, sheet, cols=10, report=os.path.join(a.out, "sfx_report.json"))
